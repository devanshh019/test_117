...
_target_plot_path = r'/Users/devanshkumarverma/Desktop/PS117-v2.0.0/backend/data/storage/plot_exec_sim_repaired_1788199059682.png'
...
# Assuming P, R, S, and E are defined elsewhere and are numpy arrays
# For demonstration purposes, let's define them as numpy arrays
P = np.array([1.0, 2.0, 3.0])
R = np.array([4.0, 5.0, 6.0])
S = np.array([7.0, 8.0, 9.0])
E = np.array([10.0, 11.0, 12.0])

t_min = (P * R) / (S * E - 0.6 * P)

print(t_min)

# Example usage with plotting (demonstrating the fix)
plt.plot(P, R)
plt.axvline(x=t_min, color='red', linestyle='--', label='Limit')
plt.xlabel('P')
plt.ylabel('R')
plt.title('P vs R with Limit')
plt.legend()
plt.show()
